...
filename_pose = 'C:\Users\Simone\Dropbox\Abschlussarbeit\PEM\4-Results\graph_localization\tuned_gmap\f2\pose.csv';
filename_odom = 'C:\Users\Simone\Dropbox\Abschlussarbeit\PEM\4-Results\graph_localization\tuned_gmap\f2\odom.csv';
delimiter = ';';
startRow = 2;
...
if(positionx_pose(index,1)>= Ax(1,j) - 0.11 && positionx_pose(index,1)<= Ax(1,j) + 0.11)
...
for index = 135:205 %look at fieldposeposepositiony_pose index 0:13 meters
    
if(positiony_pose(index,1)>= By(1,j) - 0.11 && positiony_pose(index,1)<= By(1,j) + 0.11)
...
for index = 210:274 % look at fieldposeposepositionx_pose(index) --> 37: 27 meters
...
for index = 220:290 %look at fieldposeposepositiony_pose index --> 13:8 meters
    
if(positiony_pose(index,1)>= Dy(1,j) - 0.11 && positiony_pose(index,1)<= Dy(1,j) + 0.11)
